...
from phyloformer.training import init_training, load_checkpoint, save_checkpoint
import random
import torch
...
def train(device: str, wandb_mode: str):
    with wandb.init(mode=wandb_mode) as wandb_run:
        ...
        if config.checkpoint:
            ...
            in_channels, seq_len, n_seqs = train_data.dataset[0][0].shape
            model = AttentionNet(
                in_channels=in_channels, seq_len=seq_len, n_seqs=n_seqs,
                device=device, 
