...
from phyloformer.utils import println
...
def load_model(path: str, device: str = "cpu", single_gpu: bool = True) -> AttentionNet:
    ...
    if loaded['model'].get("state_dict") is None or loaded['model'].get("architecture") is None:
        raise ValueError(
    ...
    model = AttentionNet(**loaded['model']["architecture"], device=device)
    _init_model(model, loaded['model']["state_dict"], single_gpu)
    model.to(device)
